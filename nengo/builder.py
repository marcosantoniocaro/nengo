...
class Builder(object):
    ...
    def __call__(self, model, dt):
        ...
        for target, copytarget in zip(model.probed, self.model.probed):
            if not isinstance(self.model.probed[copytarget], SimulatorProbe):
                self.builders[nengo.Probe](self.model.probed[copytarget])
                self.probemap[model.probed[target]] = self.model.probed[
                    copytarget].probe
    ...
    def build_ensemble(self, ens):
        ...
        ens.eval_points = eval_points  # XXX ens modified
        ...
        self.data['gain'][ens.neurons] = ens.neurons.gain
        self.data['bias'][ens.neurons] = ens.neurons.bias
        if (self.data['gain'][ens.neurons] is None
                or self.data['bias'][ens.neurons] is None):
            ...
            self.data['gain'][ens.neurons], self.data['bias'][ens.neurons] = (
        ...
        self.data['encoders'][ens] = encoders

        if isinstance(ens.neurons, nengo.Direct):
            scaled_encoders = encoders
        else:
            scaled_encoders = encoders * (self.data['gain'][ens.neurons]
    ...
    def build_node(self, node):
        ...
        if node.output is None:
            ...
            self.sig_out[node] = self.sig_out[pyfn]
    ...
    def build_probe(self, probe):
        # Set up signal
        self.sig_in[probe] = Signal(np.zeros(probe.dimensions),
                                    name=probe.label)

        # reset input signal to 0 each timestep
        self.operators.append(Reset(self.sig_in[probe]))
    ...
    def build_connection(self, conn):
        ...
        if (isinstance(conn.pre, nengo.Ensemble)
            ...
            x = np.dot(conn.eval_points,
                       self.data['encoders'][conn.pre].T / conn.pre.radius)
            activities = self.dt * conn.pre.neurons.rates(
                x, self.data['gain'][conn.pre.neurons],
                self.data['bias'][conn.pre.neurons])
            if conn.function is None:
            ...
            if conn.filter is not None and conn.filter > self.dt:
                o_coef, n_coef = self.filter_coefs(
                    pstc=conn.filter, dt=self.dt)
                decoder_signal = Signal(decoders * n_coef,
            ...
            self.operators.append(ProdUpdate(
                ...
                Signal(o_coef, name="o_coef"),
                signal,
                tag=conn.label + " decoding"))
        elif conn.filter is not None and conn.filter > self.dt:
            # 4. Filtered connection
            signal = self._filtered_signal(self.sig_in[conn], conn.filter)
        else:
            # 5. Direct connection
            signal = self.sig_in[conn]
        ...
        if isinstance(conn.post, nengo.objects.Neurons):
            transform *= self.data['gain'][conn.post][:, np.newaxis]
    ...
    def build_pyfunc(self, pyfn):
        if pyfn.n_in > 0:
            self.sig_in[pyfn] = Signal(np.zeros(pyfn.n_in),
                                       name=pyfn.label + '.input')
            self.operators.append(Reset(self.sig_in[pyfn]))
        self.sig_out[pyfn] = Signal(np.zeros(pyfn.n_out),
                                    name=pyfn.label + '.output')
        self.operators.append(
            SimPyFunc(output=self.sig_out[pyfn],
                      J=self.sig_in[pyfn] if pyfn.n_in > 0 else None,
                      fn=pyfn.fn,
                      n_args=pyfn.n_args))
    ...
    def build_neurons(self, neurons):
        ...
        bias_signal = Signal(self.data['bias'][neurons],
